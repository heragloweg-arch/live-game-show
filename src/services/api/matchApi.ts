...
async function invoke<T>(fnName: string, body: Record<string, unknown>): Promise<T> {
  ...
  try {
    ...
    throw new Error(
      res.ok
        ? `Function ${fnName} returned non-JSON`
        : `Function ${fnName} failed (${res.status}) — non-JSON body`
    );
  }
  if (!res.ok) {
    const msg = (data && (data.error || data.message)) || `Function ${fnName} failed (${res.status})`;
    throw new Error(typeof msg === 'string' ? msg : `Function ${fnName} failed`);
  }
  return data as T;
}
